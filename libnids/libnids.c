...
struct nids_prm nids_params = {
    ...
    0,				/* tcp_workarounds */
    NULL,			/* pcap_desc */
	/*handong add used for log output*/
	0          /* output to 0 default */
	/*~end*/
};
...
static void nids_syslog(int type, int errnum, struct ip *iph, void *data)
{
...
#ifdef WIN32
	    fprintf(nids_params.out, "%s,from %s to %s\n",
		   nids_warnings[errnum], saddr, daddr);
#else
    ...
    default:
#ifdef WIN32
	fprintf(nids_params.out, "Unknown warning number ?\n");
#else
